...
async function loadCampsData() {
    try {
        const response = await fetch('school-district-camps.json');
        ...
        console.error('Error loading school district data:', error);
        document.getElementById('campsContainer').innerHTML = 
            '<div class="no-results"><p>Error loading school district data. Please refresh the page.</p></div>';
    }
}
...
async function getCampCoordinates(camp) {
    ...
    // Known coordinates for school district locations
    const knownLocations = {
        ...
        'Navarro': [30.2600, -97.7300],
        'Travis': [30.2800, -97.7200],
        'Eastside': [30.2700, -97.7100],
        'Round Rock ISD': [30.5083, -97.6789],
        'Austin ISD': [30.2672, -97.7431]
    };
    
    // Check for known locations
    for (const [key, coords] of Object.entries(knownLocations)) {
        if (location.includes(key) || camp.name.includes(key)) {
...
function createMapPopup(camp) {
    ...
    return `
        <div class="map-popup">
            ...
            ${camp.district ? `<div class="popup-info"><strong>District:</strong> ${escapeHtml(camp.district)}</div>` : ''}
...
function createCampCard(camp) {
    ...
    return `
        <div class="camp-card">
            ...
            ${camp.district ? `<span class="camp-type" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">${escapeHtml(camp.district)}</span>` : ''}
...
function filterCamps() {
    ...
    filteredCamps = campsData.filter(camp => {
        ...
        const matchesSearch = !searchTerm || 
            ...
            (camp.district && camp.district.toLowerCase().includes(searchTerm)) ||
        ...
        return matchesSearch && matchesDistrict && matchesAge;
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('districtFilter').addEventListener('change', filterCamps);
